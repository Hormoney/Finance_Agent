...
template1 = PromptTemplate(
    template='''
    # 角色
    你是一位资深的财务分析师，善于通过企业的主要财务指标表、利润表来分析企业的盈利能力，你有非常精确的信息匹配和抽取能力。。

    # 目标
    通过提供的主要财务指标表、利润表，结合你的金融领域知识，计算用于评估企业盈利能力的财务指标，并与前两年的数据进行对比，从而揭示出企业的风险点所在。
    ...
    # 工作流程
    1.接收财表数据和领域知识
    2.根据实际情况，从输入数据中抽取分析企业盈利能力所需的数据信息
    3.整理数据信息抽取结果，输出你从输入中筛选抽取出的用于计算企业盈利能力分析关键数据信息。

    # 限制
    仅输出用于计算企业盈利能力分析关键数据信息，用markdown表格的形式输出。
...
# 根据template3的结果，进行盈利能力分析所需的指标计算

template4= PromptTemplate(
    template='''
    # 角色
    你是一位资深的财务分析师，善于通过企业的主要财务指标表、资产负债表来分析企业的盈利能力。
    ...
    4.整理数据分析结果，输出你的财务分析步骤和计算出的用于企业盈利能力分析的财务指标。

    # 限制
    仅输出你的财务分析步骤和计算出的用于企业盈利能力分析的财务指标。
...
template5 = PromptTemplate(
    ...
    # 工作流程
    1.接收财务指标计算结果和领域知识
    2.结合计算出的指标和你的知识，揭露企业在盈利能力上可能出现的风险点。

    # 限制
    仅输出该企业的盈利能力分析及该企业在盈利能力上可能出现的风险点
...
# 封装成一个工作流
def calculate_profit_ability(input_data):
    ...
    # 第四步：模型2解释计算结果并进行盈利能力分析所需指标计算
    explanation = model4(judge_result)
    
    # 第五步：模型3解释计算结果并进行盈利能力分析
...
# 使用工作流
input_data = '主要指标表数据+利润表数据'
results = calculate_profit_ability(input_data)
print("信息抽取结果:", results["judge_result"])
print("指标计算结果:", results["explanation"])
print("盈利能力分析:", results["calculate"])
